...
//
// This mirrors functions/api/chat.js but runs as a standalone Worker so the
// chatbot also works on the static GitHub Pages deployment (github.io cannot
// run Pages Functions). The Workers AI key stays server-side.
//
...
export default {
  async fetch(request, env) {
    ...
    if (!accountId || !apiToken) {
      return json(
        { error: "AI is not configured. Add CF_AI_ACCOUNT_ID and CF_AI_API_TOKEN to the Worker." },
        500
      );
    }
